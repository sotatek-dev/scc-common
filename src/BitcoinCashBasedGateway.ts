...
export abstract class BitcoinCashBasedGateway extends BaseGateway {
  // public static convertInsightUtxoToBitcoreUtxo(utxos: IBCHInsightUtxoInfo[]): IBCHBitcoreUtxoInput[] {
  ...
  protected async _fetchOneBlockTxsInsightPage(
    ...
    // const result1 = data.map(tx => {
    //   const txRaw = this.getOneTransaction(tx);
    //   // Check whether a transaction is Omni
    //   const isOmniTx: boolean = tx.outputs.some(vout => {
    ...
    //   }
    //
    //   return new UTXOBCHBasedTransaction(currency, tx, block);
    // });
